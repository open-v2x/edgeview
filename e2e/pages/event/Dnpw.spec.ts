...
test.describe('The Sds Page', () => {
  ...
  test('用路侧模拟器发送[逆向超车预警]数据', async ({ page }) => {
    await page.goto(`${baseURL}:6688`);
    await connectMqtt(page);
    await checkDataset(page, 'DNP_track');
    await checkDataset(page, 'msg_VIR_DNP');
    // 直到loading结束再点击发送按钮
    await page.locator('#loading-DNP_track').waitFor({ state: 'hidden' });
